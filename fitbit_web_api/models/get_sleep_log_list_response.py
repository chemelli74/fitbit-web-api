...
class GetSleepLogListResponse(BaseModel):
    """
    GetSleepLogListResponse
    """

    pagination: Optional[GetSleepLogListResponsePagination] = None
    sleep: Optional[List[SleepLog]] = Field(
        default=None, description="Array of sleep log entries."
    )
    summary: Optional[SleepSummary] = None
    __properties: ClassVar[List[str]] = ["pagination", "sleep", "summary"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in sleep (list)
        _items = []
        if self.sleep:
            for _item_sleep in self.sleep:
                if _item_sleep:
                    _items.append(_item_sleep.to_dict())
            _dict["sleep"] = _items
